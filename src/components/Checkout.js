...
const Checkout = () => {
  ...
  const validateRequest = (items, addresses) => {
    ...
    if(!addresses.all.length){
      enqueueSnackbar("Please add a new addresses before proceeding",{variant:"warning"})
      return false;
    }

    if(!addresses.selected.length){
      enqueueSnackbar("Please select one Shipping address tp proceed..",{variant:"warning"})
      return false;
    }
    return true;
  };
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try {
      ...
      enqueueSnackbar("Order plcaed successfully?",{variant:"success"});
