...
const Login = () => {
  ...
  const persistLogin = (token, username, balance) => {
    try {
      ...
      console.log('-----> Error saving login details to local storage');
      ...
      <Box className="content">
        <Stack spacing={2} className="form">
          ...
          {isLoading ? <Box sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}><CircularProgress /></Box> :
            <Button variant="contained" onClick={async () => await login(formData)}>LOGIN TO QKART</Button>}
          {/* <p>Don’t have an account?{" "}<a className="link" href="/register">Register now</a></p> */}
